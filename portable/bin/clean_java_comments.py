    ...
    for filename in options['filenames']:
        ...
        for line_comment in line_comments:
            ...
            if line_comment != cleaned_line_comment:
                ...
                contents = re.sub(safe_find, cleaned_line_comment, contents, flags=re.MULTILINE)
...
def clean_line_comment(comment):
    ...
    matches = re.match(r'^(\s*)(//)([\\s]*)(.*)', comment, flags=re.DOTALL | re.MULTILINE)
    if not matches:
        return comment
...
def clean_block_comment(comment):
    ...
    if cleaned_lines:
        if len(cleaned_lines) == 1:
            if opener == '/**':
                return '%s%s%s %s %s' % (newlines, indent, opener, cleaned_lines[0].strip(), closer)
            else:
                return '%s%s// %s' % (newlines, indent, cleaned_lines[0].strip())
        else:
            clean_body = "".join(("\n%s * %s" % (indent, cleaned_line)).rstrip() for cleaned_line in cleaned_lines)
            return '%s%s%s%s\n%s %s' % (newlines, indent, opener, clean_body, indent, closer)
    else:
        return newlines
